...
async function status(req, res, next) {
  ...
  if (!Array.isArray(result.data.response.message)) {
    ...
    if (!Boolean(fs_result.data.message)) {
      const error = new Error('bad command')
      return next(error)
    } else {
...
async function get_time(req, res, next) {
  ...
  if (!Array.isArray(result.data.response.message)) {
    ...
    if (!Boolean(fs_result.data.message)) {
      ...
      const params = request.gen_params('get_time_and_date', req, fiscal_response.data.message);
...
async function set_time(req, res, next) {
  ...
  if (!Array.isArray(result.data.response.message)) {
    ...
    if (!Boolean(fs_result.data.message)) {
      ...
      const params = request.gen_params('set_time_and_date', req, fiscal_response.data.message);
      const time_body = request.gen_body(params);
      const time = await axios.post(baseURL, time_body)
